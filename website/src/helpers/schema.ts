...
async function fetchDictionary(version) {
  try {
    const dict = await axios.get(`/validation/website/static/data/schemas/${version}.json`);
...
async function fetchDiff(version, diffVersion) {
  try {
    const response = await axios.get(
      `/validation/website/static/data/schemas/diffs/${diffVersion}/${diffVersion}-diff-${version}.json`,
    );
